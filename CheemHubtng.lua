...
function CheemCore.ResolveWeaponType()
    ...
    -- manual select ưu tiên trước
    if WeaponRuntime.SelectedType
    and weapons[WeaponRuntime.SelectedType] then
...
function CheemCore.UpdateEquipState(now)
    ...
    if now - WeaponRuntime.LastEquip < WeaponRuntime.EquipDelay then
        return
    end
...
function CheemCore.UpdateQuest(now)
    ...
    local lvObj = data:FindFirstChild("Level")
    if not lvObj then return end

    local newQuest = CheemCore.GetQuestByLevel(lvObj.Value)
    if not newQuest then return end

    if not Runtime.CurrentQuest
    or Runtime.CurrentQuest.MobName ~= newQuest.MobName then
        ...
        if Flags.Debug then
            warn("New quest ->", newQuest.MobName)
        end
    end
end
...
function CheemCore.FarmTick(dt)
    ...
    or Runtime.Target.Humanoid.Health <= 0 then
        if now - Runtime.LastScan > 0.15 then
    ...
    -- Hold position
    -- đứng trên đầu mob
local attackCF = hrp.CFrame * CFrame.new(0, 6, 0)
...
CreateDropdown(SettingTab, "Select Weapon Type",
function()
    return WeaponTypes
end,
function(value)
...
RunService.Heartbeat:Connect(function(dt)
    ...
    if Flags.Debug then
        print(
            "Mob:", FarmConfig.MobName,
            "Target:", Runtime.Target and Runtime.Target.Name,
            "Weapon:", WeaponRuntime.SelectedType
        )
    end
